...
--
-- Host: 127.0.0.1:3306
-- Generation Time: Apr 05, 2018 at 10:44 AM
...
CREATE TABLE IF NOT EXISTS `decors` (
  ...
  `decorType` enum('utensils','furnishing','trinkets') NOT NULL,
...
INSERT INTO `employees` (`employeeID`, `firstName`, `midName`, `lastName`, `address`, `email`, `photo`, `username`, `password`, `contactNumber`, `role`, `status`) VALUES
(0001, 'admin', 'admin', 'admin', 'admin', 'admin@yahoo.com', NULL, 'admin', 'admin', '09260878700', 'admin', 'inactive'),
(0002, 'admin', 'admin', 'admin', 'admin', 'admin@yahoo.com', NULL, '0002admin', '$2y$10$FfF.cvImguVPkaAAPSpdWOQ5Wiv3m9yY5I1yLd5acSe', '111111111', 'admin', 'active');
...
CREATE TABLE IF NOT EXISTS `entouragedetails` (
  `entourageID` int(10) UNSIGNED ZEROFILL NOT NULL,
  `designID` int(10) UNSIGNED ZEROFILL NOT NULL,
...
CREATE TABLE IF NOT EXISTS `eventthemes` (
  ...
  `themeID` int(11) UNSIGNED ZEROFILL NOT NULL,
...
--
-- Table structure for table `themedecor`
--

DROP TABLE IF EXISTS `themedecor`;
CREATE TABLE IF NOT EXISTS `themedecor` (
  `themeID` int(7) UNSIGNED ZEROFILL NOT NULL,
  `decorID` int(7) UNSIGNED ZEROFILL NOT NULL,
  PRIMARY KEY (`themeID`,`decorID`),
  KEY `fk_td_decorID` (`decorID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

-- --------------------------------------------------------

--
-- Table structure for table `themedesign`
--

DROP TABLE IF EXISTS `themedesign`;
CREATE TABLE IF NOT EXISTS `themedesign` (
  `themeID` int(7) UNSIGNED ZEROFILL NOT NULL,
  `designID` int(7) UNSIGNED ZEROFILL NOT NULL,
  PRIMARY KEY (`themeID`,`designID`),
  KEY `fk_tde_designID` (`designID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;
...
--
-- Constraints for table `themedecor`
--
ALTER TABLE `themedecor`
  ADD CONSTRAINT `fk_td_decorID` FOREIGN KEY (`decorID`) REFERENCES `decors` (`decorsID`) ON DELETE CASCADE ON UPDATE CASCADE,
  ADD CONSTRAINT `fk_td_themeID` FOREIGN KEY (`themeID`) REFERENCES `theme` (`themeID`);

--
-- Constraints for table `themedesign`
--
ALTER TABLE `themedesign`
  ADD CONSTRAINT `fk_tde_designID` FOREIGN KEY (`designID`) REFERENCES `designs` (`designID`) ON DELETE CASCADE ON UPDATE CASCADE,
  ADD CONSTRAINT `fk_tde_themeID` FOREIGN KEY (`themeID`) REFERENCES `theme` (`themeID`);
